...
def conv_op(input_op, name, kh, kw, n_out, dh, dw, p):
    """create the conv func.
        Args:
            input_op: Images Tensor
            name: conv name
            kh: kernel height
            kw: kernel weight
            n_out: kernel numbers
            dh: strides'hight
            dw: strides'weight
            p: param list
...
def inference_op(input_op, keep_prob):
    '''
        create the VggNet-16
    '''
    p = []
    
    # conv1 contains two conv layers and one pool layer 
    # in    224*224*3
    # out   112*112*64
    ...
    # reshape pool5 to [batch_size, 7*7*512]
    shp = pool5.get_shape()
