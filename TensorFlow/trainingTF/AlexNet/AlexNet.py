...
def inference(images):
    ...
    with tf.name_scope('fc2') as scope:
        ...
        parameters += [weights, biases]
        print_activations(local7)
        
    with tf.name_scope('out') as scope:
        weights = tf.Variable(tf.truncated_normal([4096,1000],
                        dtype=tf.float32, stddev=0.1), name='weights')
        biases = tf.Variable(tf.constant(0.0, shape=[1000],
                        dtype=tf.float32), trainable=True, name='biases')
        out = tf.add(tf.matmul(local7, weights), biases)
        parameters += [weights, biases]
